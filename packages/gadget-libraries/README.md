...
Workshop backend's blueprint build (`scripts/format-blueprint-files.ts`) resolves each to the
library's entry here and inlines what the entry uses into the `client.js` / `server.js` the
blueprint ships, the way it inlines a blueprint's own `lib/` modules. The archive stays
self-contained, and a gadget created from the blueprint carries its own copy of the library as of
its creation; nothing resolves the specifier at runtime, and a gadget the agent writes cannot import
one.

Nothing in this package is deployed on its own, and no package imports it: the blueprint build and
the type-check programs reach it by path.
