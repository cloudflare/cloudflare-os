...
export const REQUEST_CONNECTION_TOOL_DESCRIPTION = `
Ask the user to connect a gatekeeper resource (e.g. a ClickHouse cluster, a GitHub repo). Pre-configure as much as you can: always pass vendorId, and pass resourceUrl when you can infer a concrete URL (use listConnectableResources to learn the URL patterns). If you pass resourceUrl, it must match one of the vendor's patterns or the call is rejected with guidance and no card is shown. If you do not know the exact resource URL, omit resourceUrl; the accept flow will show the user that vendor's explicit resource picker and reuse an already-connected account when possible. Do not ask the user to manually paste a URL just so you can call this tool, and do not request a duplicate OAuth flow when the user already has a suitable connected account. You also choose \`bindingName\`: the name the resource will have in your env once connected (you know why you want the resource, so pick a name that reflects its role). On success this shows the user an accept/deny card in the chat. It does NOT block: your turn ends after a successful call, and you will be resumed once the user accepts (the resource becomes available as \`env.<bindingName>\`, which you can describeBinding and use from executeCode; wire it into a Gadget with setGadgetBinding only if the Gadget's code needs it) or denies (your turn simply ends; wait for the user's next message).
`.trim();
