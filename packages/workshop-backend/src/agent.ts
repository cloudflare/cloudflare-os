  ...
  let tools: Record<string, AgentTool> = {
    ...
    describeBinding: defineTool({
      ...
      execute: async (toolCallId, input) => {
        try {
          // Recorded already bounded, like grep: the log holds exactly what the model saw.
          let output = boundToolResultText(
              await describeBinding(input, chatBindings, chatId, hooks));
          return toolResult(output, {output} as Partial<AiToolCall>);
        } catch (error) {
          toolCallNotes.set(toolCallId, {
            error: toolErrorText(error)
