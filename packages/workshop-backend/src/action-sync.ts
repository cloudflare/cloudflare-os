...
type SyncIntent = { manualApprovals: ManualApproval[] };
...
export class ActionSyncDriver {
  ...
  sync(gatekeeperId: number, manualApproval?: ManualApproval): Promise<number[]> {
    ...
    if (!slot) {
      let resolve!: (decided: number[]) => void;
      let reject!: (error: unknown) => void;
      let promise = new Promise<number[]>((res, rej) => { resolve = res; reject = rej; });
      slot = { intent: { manualApprovals: [] }, resolve, reject, promise };
      this.#staged.set(gatekeeperId, slot);
    }
    if (manualApproval) slot.intent.manualApprovals.push(manualApproval);
  ...
  async #run(gatekeeperId: number): Promise<void> {
    try {
      for (;;) {
        ...
        try {
          slot.resolve(await this.#syncOnce(gatekeeperId, slot.intent));
        } catch (error) {
          slot.reject(error);
        }
      }
  ...
  async #syncOnce(gatekeeperId: number, intent: SyncIntent): Promise<number[]> {
    ...
    // automatic.
    let manualAscending = intent.manualApprovals.toSorted((a, b) => a.frontier - b.frontier);
    ...
    for (let record of pending) {
      if (record.action <= frontier) {
        // Covered by a manual approval; the smallest covering frontier's user takes
        // responsibility for this earlier action riding along.
    ...
    // Cascade invalidations: display-attributed to the veto that caused them, resolved by the
    // user whose rejection it was.
