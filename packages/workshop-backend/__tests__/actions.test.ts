...
import {
  ActionSyncDriver, ActionSyncStorage, GatekeeperActionTarget, autoApprovalRule, isMethodMissing,
} from "../src/actions.js";
...
import type {
  ActionDescription, ApplyActionsThroughResult,
...
describe("ActionSyncDriver.apply", () => {
  ...
  it("suspends every rule while the workspace is latched, riding nothing along with a click",
     async () => {
    let storage = makeStorage();
    enableRule(storage);
    putAction(storage, 1);                             // rule-authorized, were it not latched
    putAction(storage, 2);
    storage.containsRestrictedData.put(true);

    let { target, calls } = makeBatchGatekeeper();
    let driver = makeDriver(storage, target);
    await driver.apply(GK);
    let { decided } = await driver.apply(GK, { action: 2, resolvedBy: APPROVER });

    // Nothing applies without a human: the rule neither fires on its own nor carries action 1
    // along under the click on action 2, where it stands as an undecided gate instead.
    expect(decided).toEqual([]);
    expect(calls).toEqual([]);
    for (let action of [1, 2]) expect(getAction(storage, action).state).toBe("pending");
  });
...
describe("Overseer auto-approval dispatch", () => {
  it("starts the apply pass only after submitAction returns", async () => {
    ...
    await runInDurableObject(stub, async (instance: OverseerDurableObject) => {
      ...
      // submitAction refuses an action naming a connection the workspace doesn't have.
      impl.storage.gatekeepers.put({ id: GK, resourceTitle: "Connection", class: {} });
