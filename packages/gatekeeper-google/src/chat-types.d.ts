...
export type GoogleChatMessageInfo = {
   ...
   * message has no real Chat resource name yet, so it cannot be replied to, edited, or
   * reacted to until it has been committed — and its `createTime` is provisional: the committed
   * message carries the timestamp Google assigns at commit.
   */
  pending?: boolean;
};
 ...
 * minutes. Use `GoogleChatSpace.listMessages()` when complete, current history for one known
...
export interface GoogleChatSpace {
   ...
   */
  sendMessage(text: string): Promise<GoogleChatMessage>;
}
...
export interface GoogleChatMessage {
   ...
   */
  reply(text: string): Promise<GoogleChatMessage>;
