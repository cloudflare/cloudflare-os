...
/** Whether a note is a person's comment on the thread (not GitLab activity, not a diff comment). */
export function isDiscussionComment(note: GitLabNoteResponse): boolean {
  return !note.system && note.type !== "DiffNote";
}
...
export function commentTargetFromPosition(position: GitLabPositionResponse): GitLabDiffCommentTarget {
  ...
  const endSide = position.new_line != null ? "new" : "old";
  const target: GitLabDiffCommentTarget = {
    path,
    subjectType: "line",
    line: (endSide === "new" ? position.new_line : position.old_line) ?? 1,
    side: endSide,
  };
  if (range?.start) {
    const startSide = range.start.type === "old" ? "old" : "new";
    const startLine = startSide === "new" ? range.start.new_line : range.start.old_line;
    if (startLine != null && (startLine !== target.line || startSide !== target.side)) {
      target.startLine = startLine;
      target.startSide = startSide;
    }
  }
  return target;
}
 ...
 * items into a streamed listing at the right positions. `popularity` is left to the remote
 * order (injected items sort as if they had no votes, i.e. by creation).
 */
export function issuableComparator<T extends { id: string; createdAt: Date; updatedAt: Date }>(
  sort: "created" | "updated" | "popularity" | undefined,
  direction: "asc" | "desc" | undefined,
): (a: T, b: T) => number {
  const factor = (direction ?? "desc") === "asc" ? 1 : -1;
  return (a, b) => {
    let delta = sort === "updated"
