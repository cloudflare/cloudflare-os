...
export class UserAccount extends DurableObject<Env> implements AccountCredentials {
  ...
  /**
   * The current access token with the id of the grant it belongs to, refreshed when it is within
   * the safety window of expiry. The fast path is outside the lock; refresh is serialized and
   * re-checked inside it, and the rotated pair is written in one transaction before any caller
   * sees the new token. The grant id travels with the token so that what GitLab says about the
   * token -- a 401 -- can be reported against the grant that produced it (`credentialsRejected`).
   */
  async getCredential(): Promise<GitLabCredential> {
    ...
    if (this.#tokenIsFresh(grant)) return { token: grant.accessToken, grantId: this.#grantId() };
    ...
    return await this.#credentials.run(async () => {
      ...
      const grantId = this.#grantId();
      if (this.#tokenIsFresh(current)) return { token: current.accessToken, grantId };
      ...
      if (!result.ok) {
        ...
        await this.credentialsRejected(grantId);
        throw error;
      }
      ...
      if (this.ctx.storage.kv.get<string>("refreshToken") !== current.refreshToken) {
        ...
        if (replaced) return { token: replaced.accessToken, grantId: this.#grantId() };
      ...
      this.#mintFailure = undefined;
      return { token: result.grant.accessToken, grantId };
    });
  }
  ...
  /**
   * GitLab refused a token of grant `grantId`: tell the Workshop the account needs reconnecting,
   * once. The report is fenced to the grant -- a request that took token A, then a reconnect
   * that installed grant B while it was in flight, then A's delayed 401: that 401 says nothing
   * about B and is dropped. A credential without an id (none is ever handed out without one)
   * is reported unfenced. The notice itself is the kit's: it latches only after the Workshop
   * has acknowledged, so a callback that fails transiently is retried by the next refusal
   * rather than silenced, and a grant written during the call is not latched against.
   */
  async credentialsRejected(grantId: string | undefined): Promise<void> {
    if (grantId !== undefined && this.ctx.storage.kv.get<string>("grantId") !== grantId) return;
    const callback = this.ctx.storage.kv.get<Fetcher<GatekeeperConnectCallback>>("callback");
