...
export class GitLabGatekeeperImpl extends DurableObject<Env, GitLabGatekeeperImplProps>
   ...
   * keeps the review's `approve` honest the same way, answering 409 to a stale `sha`, but only
   * on the attempt that runs it; a retry after a push would otherwise resume past it, and
   * GitLab may have reset the approval on that push). The check reads the live merge request,
   * not the cache, since it is what the action is bound to.
   *
   ...
   * makes multi-call. Approval runs before anything is posted, so a stale head -- or any other
   * refusal of the approval itself -- fails the action with nothing published. A retry then
   * skips the comments already published, deletes the drafts it created but never published
   * (GitLab keeps them parked, visible only to the user; mistaking them for the user's would
   * send the retry down the individual-publish path below and lose the reviewer state),
  ...
  async #publishReview(record: StoredActionRecord, action: PostReviewAction): Promise<void> {
    ...
    if (live.sha !== review.revision.headSha) {
      throw new Error(
        `Merge request !${realId} has moved on from the reviewed revision (${review.revision.headSha.slice(0, 12)} -> ` +
        `${live.sha.slice(0, 12)}); the review's comments and approval refer to code that has since changed. ` +
        "Re-read the diff and review it again.");
    }
    ...
    // drafts created and recorded, which the retry clears. Every page: `bulk_publish` publishes
    // every draft the user has, so a human's draft on page two, behind a page of ours, would be
    // published on the strength of a first page that showed none.
    const foreignDrafts = async (ours: Set<number>) => {
      const drafts = await this.#fetchAllPages((page, perPage) => this.#withApi(api => api.listDraftNotes(projectPath, iid, page, perPage)));
      const foreign = drafts.filter(draft => !ours.has(draft.id));
