...
export class GitLabApiError extends Error {
  ...
  isAuthError: boolean;
  /** The grant whose token the failed request carried, when the credential named one. */
  grantId?: string;
  movedTo?: string;

  constructor(status: number, message: string,
              options: { details?: unknown; movedTo?: string; isAuthError?: boolean; grantId?: string } = {}) {
    super(message);
...
/**
 * A bearer token and, when the source keeps one, the id of the authorization it belongs to. A
 * request's failure is a fact about the credential it carried, so an error reports the grant id
 * back and a caller acts on it only while that grant is still the live one.
 */
export type GitLabCredential = { token: string; grantId?: string };
...
export class GitLabApi {
  ...
  /** One page of the user's draft notes on the merge request (a paginated listing like any other). */
  async listDraftNotes(projectPath: string, iid: number, page: number, perPage: number): Promise<GitLabDraftNoteResponse[]> {
    return await this.#get<GitLabDraftNoteResponse[]>(
      `/projects/${encodeProjectPath(projectPath)}/merge_requests/${iid}/draft_notes`, { page, per_page: perPage });
  }
  ...
  async #gitPost(
    ...
    if (!response.ok) {
      ...
      throw new GitLabApiError(response.status,
        ...
        { isAuthError: response.status === 401, grantId: credential.grantId });
    }
    const contentType = response.headers.get("content-type") ?? "";
