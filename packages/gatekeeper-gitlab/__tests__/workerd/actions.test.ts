...
describe("issue creation", () => {
  ...
  it("rejecting a create also cascades to actions whose text names the provisional (#~N), which could never apply", async () => {
    ...
    // both would fail every apply once #~1 can never resolve.
    ...
    await expect(unwrap(await hooks().applyAction(name, props, citing.approvalId))).rejects.toThrow(/no longer pending/);
    await expect(unwrap(await hooks().applyAction(name, props, comment.approvalId))).rejects.toThrow(/no longer pending/);
    ...
    expect(discussion.map(entry => entry.bodyMarkdown)).toEqual(["see #~2 and !~1"]);
  ...
  it("recurses the cascade through a dependent create: a comment on an issue that cited a rejected MR is retired too", async () => {
    ...
    expect(await unwrapOutcome(await hooks().rejectAction(name, props, mr.approvalId))).toEqual({ restart: true });
    await expect(unwrap(await hooks().openIssue(name, props, "~2"))).rejects.toThrow(/No provisional issue exists/);
    await expect(unwrap(await hooks().applyAction(name, props, onIssue.approvalId))).rejects.toThrow(/no longer pending/);
  });
});
...
describe("merge requests", () => {
  ...
  it("names an unchanged line by both sides, an added line by new_line, and a removed line by old_line", async () => {
    ...
    const review = {
      ...
      diffComments: [
        ...
        { target: { ...target("old", 11), startLine: 13, startSide: "new" as const }, bodyMarkdown: "mixed-side range" },
      ],
    };
    const action = await unwrap(await hooks().queueAction(name, props, "preparePostReview", ["133", review], DESC));
    await unwrapOutcome(await hooks().applyAction(name, props, action.approvalId));
    ...
    expect(drafts[5].position.line_range).toMatchObject({ start: { type: "new" }, end: { type: "old" } });
  });
  ...
  it("is idempotent under retry: a stale head fails before anything is posted, and a landed approval is neither repeated nor left behind", async () => {
    ...
    // The merge request's live head: the reviewed one, until a push moves it.
    let liveSha = MR.sha;
    withMergeRequest(gitlab);
    gitlab.on("GET", new RegExp(`^/api/v4/projects/${P}/merge_requests/133\\?`), () => json({ ...MR, sha: liveSha }));
    ...
    // 1. The head moved before the first attempt: the live read catches it and nothing is
    //    attempted, twice. Were the read to miss it, approve's own `sha` CAS (409) is GitLab's
    //    second line -- exercised here by a fake that still lists the reviewed head.
    liveSha = "f".repeat(40);
    for (let attempt = 0; attempt < 2; attempt++) {
      await expect(unwrapOutcome(await hooks().applyAction(name, props, action.approvalId))).rejects.toThrow(/moved on from the reviewed revision/);
    }
    ...
    //    A push lands between the failure and the retry. The approval is recorded as done, so
    //    nothing would re-check the head through the CAS; the live read does, and the retry
    //    refuses before publishing comments positioned in a diff that no longer exists.
    liveSha = "e".repeat(40);
    const publishesBefore = gitlab.count("POST", /bulk_publish$/);
    await expect(unwrapOutcome(await hooks().applyAction(name, props, action.approvalId))).rejects.toThrow(/moved on from the reviewed revision/);
    expect(parked).toEqual([1]);
    expect(gitlab.count("POST", /bulk_publish$/)).toBe(publishesBefore);
    liveSha = MR.sha;
    publishStatus = 200;
    await unwrapOutcome(await hooks().applyAction(name, props, action.approvalId));
    expect(gitlab.count("POST", /\/approve$/)).toBe(2);
  ...
  it("finds a foreign draft on a later page: bulk_publish would publish it, so every page is read first", async () => {
    const { gitlab, props, name } = await setup("foreign-drafts-paged");
    withMergeRequest(gitlab);
    gitlab.on("GET", new RegExp(`^/api/v4/projects/${P}/merge_requests/133/discussions`), () => json([]));
    // The user's parked draft (id 500) is older than ours, so GitLab lists it last; with a full
    // page of ours ahead of it, a first-page read shows only our own drafts.
    const parked: number[] = [];
    let nextDraftId = 1;
    gitlab.on("GET", new RegExp(`^/api/v4/projects/${P}/merge_requests/133/draft_notes\\?`), request => {
      const all = [...parked, 500];
      const page = Number(request.url.searchParams.get("page"));
      const perPage = Number(request.url.searchParams.get("per_page"));
      return json(all.slice((page - 1) * perPage, page * perPage).map(id => ({ ...fx.draftNotesResponse.data[0], id })));
    });
    gitlab.on("POST", new RegExp(`^/api/v4/projects/${P}/merge_requests/133/draft_notes$`), () => {
      const id = nextDraftId++;
      parked.push(id);
    ...
    gitlab.on("PUT", new RegExp(`^/api/v4/projects/${P}/merge_requests/133/draft_notes/\\d+/publish$`), () => json({}));
    gitlab.on("POST", new RegExp(`^/api/v4/projects/${P}/merge_requests/133/notes$`), () => json({ ...fx.issueNotesResponse.data[1], id: 7 }, { status: 201 }));
    gitlab.on("POST", new RegExp(`^/api/v4/projects/${P}/merge_requests/133/draft_notes/bulk_publish$`), () => json({}));
    gitlab.install();
    // 100 comments: exactly one remote page of our own drafts, pushing the human's to page two.
    const diffComments = Array.from({ length: 100 }, (_, i) => ({ target: { path: "README", subjectType: "file" as const }, bodyMarkdown: `c${i}` }));
    const action = await unwrap(await hooks().queueAction(name, props, "preparePostReview", ["133", {
      revision: { baseSha: MR.diff_refs.start_sha, headSha: MR.sha }, decision: "comment", bodyMarkdown: "Summary", diffComments,
    }], DESC));
    await unwrapOutcome(await hooks().applyAction(name, props, action.approvalId));
    expect(gitlab.count("POST", /bulk_publish$/)).toBe(0);
    expect(gitlab.count("PUT", /\/publish$/)).toBe(100);
    expect(gitlab.count("POST", /merge_requests\/133\/notes$/)).toBe(1);
    // Ours were published one by one; the human's (500) was neither published nor deleted.
    expect(gitlab.count("PUT", /draft_notes\/500\/publish$/)).toBe(0);
    expect(gitlab.count("DELETE", /draft_notes\/500$/)).toBe(0);
    expect(gitlab.requests.some(r => r.url.pathname.endsWith("/draft_notes") && r.url.searchParams.get("page") === "2")).toBe(true);
  });
