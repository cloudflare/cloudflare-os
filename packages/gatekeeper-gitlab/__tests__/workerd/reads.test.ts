...
describe("merge requests", () => {
  ...
  function fakeMergeRequest(): FakeGitLab {
    ...
    fake.on("GET", new RegExp(`^/api/v4/projects/${P}/merge_requests/133/commits`), request =>
      ...
      json(request.url.searchParams.get("page") === "1"
        ...
        : []));
    fake.on("GET", new RegExp(`^/api/v4/projects/${P}/merge_requests/133/discussions`), () =>
      json([fx.diffDiscussionResponse.data, { id: "plain", individual_note: true, notes: [fx.issueNotesResponse.data[1]] }]));
    return fake;
  }
  ...
  it("groups diff discussions into threads with targets and resolution, ignoring plain notes", async () => {
    ...
    expect(threads[0]).toMatchObject({
      ...
      target: { path: "package.json", subjectType: "line", line: 27, side: "new", startLine: 10, startSide: "new" },
      isResolved: false,
      // The fixture's position head differs from the merge request's current head.
      isOutdated: true,
      comments: [{ id: "1128", bodyMarkdown: "diff comment", author: { username: "root" } }],
    });
