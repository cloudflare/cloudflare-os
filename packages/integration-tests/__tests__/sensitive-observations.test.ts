...
describe("sensitive observations", () => {
  ...
  it.concurrent("a latched connection cannot be removed while the workspace is shared",
    ...
    await withSession(async publicApi => {
      ...
      await expect(solo.session.readValue(true)).resolves.toBe(42);
  ...
  it.concurrent("ambient reconciliation preserves a shared restricted producer", async () => {
    await withSession(async publicApi => {
      ...
      await expect(ambientSession.readValue(true)).resolves.toBe(42);
      const bob = await addBob(publicApi, ws);

      // Bob verifies (against the capsule too) and stays connected, held open until the restart
      // below has been observed: the reconcile runs in the background after the owner's open
      // returns, so closing him any earlier could leave it nothing to sever.
      const bobSession = await bobHolds(ws, bob);
      try {
        ...
        // reconcile restarts the workspace out from under this connection (possibly severing this
        // very open, which is fine: the reconcile has run either way). Come back on a fresh one,
        // then wait for the replacement's record to appear (proof the reconcile has run).
        await ws.aliceApi.openGadget(ws.gadgetId).then(
            overseer => overseer[Symbol.dispose](), () => {});
        const reopened = await reopenAfterRestart(ws);
        try {
          const ids = await waitFor("the replacement ambient capsule to be provisioned",
              async () => {
