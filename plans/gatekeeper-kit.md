...
prevent. That bypass bounds the window to one activation: `authority()` is `undefined` until a
fetch succeeds, so a facet that has not yet read credentials cannot serve a hit at all. What
remains is a last-seen value going stale between an in-place reconnect and the facet's next
credential *fetch*, closed by any operation that misses, since a miss reads the account afresh
(§4.6). A hit-only workload inside that activation does not close it: it keeps serving the previous
connection's entries until their TTL, so a port whose reconnect can change provider principal calls
`invalidateAll()` from its reconnect path. An async authority was considered and rejected: a DO
