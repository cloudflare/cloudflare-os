...
- **Facts derived from a grant are fenced to it.** A connect or reconnect mints a `grantId`; a
  ...
  The same fence covers what GitLab *says* about a token: `UserAccount.getCredential()` hands out
  `{ token, grantId }`, `GitLabApi` stamps the `grantId` a failed request carried onto its
  `GitLabApiError`, and the one reporting path — `withAccountApi` in `gitlab-env.ts`, shared by
  the entrypoint and the DO — calls `credentialsRejected(grantId)`, which drops the report if
  that grant is no longer live. A 401 for token A arriving after a reconnect installed grant B
  therefore says nothing about B. The notice itself is the kit's `credential-expiry` latch
...
- **Reviews are draft notes + `bulk_publish`; all three GitHub decisions are supported.** GitLab
  ...
  didn't review. That CAS runs only on the attempt that approves, though, and every review's
  ...
  approval and publish comments into a diff that no longer exists. `decision: "comment"`
  publishes with `reviewer_state: reviewed`. So
...
- `packages/gatekeeper-kit/src/git-objects.ts` — from `git-commits.ts`: `isCommitOid`,
  ...
  `workshop-shared/gatekeeper`. One line of code changes in the move: `parseGitIdentity`'s
  pattern, `^(.*?)\s*<`, let the lazy name and `\s*` both consume the same run of spaces (CodeQL
  `js/polynomial-redos`); it is now `^([^<]*)<` with the name's trailing whitespace trimmed
  afterwards — git itself strips angle brackets from names, so nothing well-formed reads
  differently.
...
- **Reject cascades**: as GitHub's (`#rejectActionsForResource`, `#rejectReplyDependencyChain`,
  `#rejectMergeRequestsForMissingBranches`), with one addition: an action *depends* on a
  provisional resource if it targets it **or its text names it** (`#~N` / `!~N`). The text
  fields are the ones `#rewriteKnownReferences` rewrites at apply, listed once in
  ...
  every nested step) retires that provisional's dependents in turn. GitHub cascades on the
  target alone, one level (Punted).
...
- **Discussion**: `#getDiscussion` reads `…/notes` for both kinds, dropping `system: true` and
  ...
  `position_type === "file"` → file target.
...
- **Simulation**: `#simulateBranchHead`, `#collectPendingChain` (`isCommitOnRemote` via `GET
  ...
  `popularity`, so the tie-break places most of them.
...
- **Draft notes are per-user drafts on the MR.** `bulk_publish` is documented as publishing
  ...
  The foreign-draft read is a paginated listing like any other (20 per page by default), so it
  is walked to the end before the bulk path is taken: a review with a full page of its own drafts
  would otherwise hide a human's older draft on page two and publish it.
