...
| **google** | Google Chat Conversation | **B** | Check the observer's own account can open the bound space (`spaces.get`). Everything the binding reads — including the other participant's name on a direct message — comes from Chat's own space-scoped responses. |
