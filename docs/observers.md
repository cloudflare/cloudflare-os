...
| **google** | Google Chat Conversation | **C** | Check the observer's own account can open the bound space (`spaces.get`). If an unnamed DM is labeled with a People-sourced participant name, track that exact name and verify its visibility with the observer's own credentials on admission and before disclosure. Ordinary Chat data remains space-scoped. |
