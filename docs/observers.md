...
| **google** | Google Chat Conversation | **B** | Check the observer's own account can open the bound space (`spaces.get`; 401/403/404 means no). Chat permissions are per conversation and everything the binding reads — messages, members, reactions, attachments — lives inside it, so the space is the atomic unit. |
